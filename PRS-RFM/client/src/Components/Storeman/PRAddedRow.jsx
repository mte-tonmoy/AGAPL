...
const PRAddedRow = ({ product, index, refetch }) => {
  ...
  const handleAddPr = (event) => {
    ...
    console.log(prData);
    fetch(`https://prs-server-7b9t.onrender.com/addPrNo/${_id}`, {
      method: "PATCH",
      headers: {
  ...
  const handleAddDeliverQuantity = (event) => {
    ...
    fetch(
      `https://prs-server-7b9t.onrender.com/updateCurrentStock/${productId}`,
      {
        method: "PATCH",
        headers: {
          "content-type": "application/json",
      ...
      .then((data) => {
        ...
        if (data.modifiedCount) {
          ...
          fetch(
            `https://prs-server-7b9t.onrender.com/addDeliverQuantity/${_id}`,
            {
              method: "PATCH",
              headers: {
                "content-type": "application/json",
